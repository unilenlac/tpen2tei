...
>>>>>>> XML rich tokens (no decomposition)
    ...
    def _split_text_node(self, context, tnode, tokens):
        if not self.INMILESTONE:
            return tokens
        tnode = tnode.rstrip('\n')
        words = re.split('\s+(?![\s:!?]+)', tnode) # split at space, but not (?! ... ) when followed by some characters; here, :, ! and ? (Latin-specific convention)
